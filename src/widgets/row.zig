...
pub const Row = struct {
    ...
    pub fn init(config: Config) Row {
        const box = c.gtk_box_new(c.GTK_ORIENTATION_HORIZONTAL, config.spacing);

        c.gtk_box_set_homogeneous(@ptrCast(box), if (config.homogeneous) 1 else 0);

        // Main axis alignment (horizontal for row)
        const halign: c_uint = switch (config.main_align) {
        ...
        const valign: c_uint = switch (config.cross_align) {
            .start => c.GTK_ALIGN_START,
            .center => c.GTK_ALIGN_CENTER,
            .end => c.GTK_ALIGN_END,
    ...
    pub fn append(self: Row, child: *c.GtkWidget) void {
        c.gtk_box_append(@ptrCast(self.widget), child);
    }

    pub fn prepend(self: Row, child: *c.GtkWidget) void {
        c.gtk_box_prepend(@ptrCast(self.widget), child);
    }

    pub fn remove(self: Row, child: *c.GtkWidget) void {
        c.gtk_box_remove(@ptrCast(self.widget), child);
    }

    pub fn setSpacing(self: Row, spacing: c_int) void {
        c.gtk_box_set_spacing(@ptrCast(self.widget), spacing);
    }

    pub fn addCssClass(self: Row, class: [*:0]const u8) void {
        c.gtk_widget_add_css_class(self.widget, class);
    }

    pub fn asWidget(self: Row) *c.GtkWidget {
        return self.widget;
    }
};
